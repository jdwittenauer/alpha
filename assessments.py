...
def calculate_book_value(stockholder_equity, goodwill, intangible_assets, long_term_assets):
    return stockholder_equity - goodwill - intangible_assets - long_term_assets
...
def calc_EBIT(operating_income, interest_expense, non_operating_income):
    return operating_income - interest_expense + non_operating_income
...
def calc_earnings_yield(EBIT, capital_expenditures, enterprise_value):
    return float(EBIT - capital_expenditures) / float(enterprise_value)
...
def calc_invested_capital(total_assets, excess_cash, equity_investments, long_term_assets, current_liabilities):
    return total_assets - excess_cash - equity_investments - long_term_assets - current_liabilities


def calc_return_on_invested_capital(operating_income, invested_capital):
...
def calc_free_cash_flow_yield(operating_cash_flow, capital_expenditures, shares_outstanding, price_per_share):
    return (float(operating_cash_flow - capital_expenditures) / float(shares_outstanding)) / price_per_share


def main():
    # Questions
    # Should EBIT include interest expense?
    # Should earnings yield deduct capital expenditures?
    # Should working capital include excess cash and/or short-term debt?
    # Should return on capital use EBIT or operating income?
    # Should invested capital subtract intangible assets?
    ...
    equity_investments = 130162
    goodwill = 4616
    intangible_assets = 4179
    long_term_assets = 5146
    total_assets = 231839
    ...
    book_value = calculate_book_value(stockholder_equity, goodwill, intangible_assets, long_term_assets)
    print('Book Value = ' + str(book_value))

    operating_income = calc_operating_income(revenue, cost_of_goods_sold, operating_expenses)
    EBIT = calc_EBIT(operating_income, interest_expense, non_operating_income)
    ...
    earnings_yield = calc_earnings_yield(EBIT, capital_expenditures, enterprise_value)
    ...
    invested_capital = calc_invested_capital(total_assets, excess_cash, equity_investments,
                                             long_term_assets, current_liabilities)
    return_on_invested_capital = calc_return_on_invested_capital(operating_income, invested_capital)
    ...
    free_cash_flow_yield = calc_free_cash_flow_yield(operating_cash_flow, capital_expenditures,
                                                     shares_outstanding, price_per_share)
